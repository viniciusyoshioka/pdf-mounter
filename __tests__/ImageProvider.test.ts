...
describe("ImageProvider", () => {
    ...
    test("Reading portrait image not having a next portrait image", () => {
        ...
        expect(() => imageProvider.nextPortrait()).toThrow("No more portrait images read to return")
    })
    ...
    test("Reading landscape image not having a next landscape image", () => {
        expect(imageProvider.hasNextLandscape()).toBeFalsy()
        expect(() => imageProvider.nextLandscape()).toThrow("No more landscape images read to return")
    })
